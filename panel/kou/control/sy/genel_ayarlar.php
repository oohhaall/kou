...
if($sor_varmi->rowCount()>0){
	$up_gunc = $db->prepare("UPDATE $table SET iletisimGenelMail=:mail");
	$up_gunc->bindValue(":mail",$mail_adresi);
	$up_gunc->execute();
	...
	$up_gunc = $db->prepare("INSERT INTO $table(iletisimGenelMail) VALUES(:mail)");
	$up_gunc->bindValue(":mail",$mail_adresi);
	$up_gunc->execute();
