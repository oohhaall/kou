...
icerik_resim
	*/
	if(!empty($_FILES["icerik_resim"]["name"])){
		...
		$res_ekle = $db->prepare("INSERT INTO isveren_markasi(baslik,sira,resim,icerik) values(:baslik,:sira,:resim,:icerik)");
		$res_sira = $db->query("SELECT MAX(sira) FROM finans")->fetch();
      ...
      <div class="alert alert-success alert-dismissable">
		...
		$res_ekle = $db->prepare("INSERT INTO isveren_markasi(baslik,sira,resim,icerik,tur,ust_kt) values(:baslik,:sira,:resim,:icerik,:tur,:alt_kt)");
		$res_sira = $db->query("SELECT MAX(sira) FROM isveren_markasi WHERE ust_kt='$alt_kt'")->fetch();
		$res_ekle->bindValue(":baslik",$baslik);
		$res_ekle->bindValue(":icerik",$icerik);
		$res_ekle->bindValue(":resim",$isim_s);
      ...
      <div class="alert alert-success alert-dismissable">
		...
		$sor_sid = $db->query("SELECT * FROM isveren_markasi WHERE id='".$sid."'")->fetch();

		if($sor_sid["ust_kt"] == 0){
			@unlink($file_upload_path.$sor_sid["resim"]);

			$sor_ana_sid = $db->query("SELECT * FROM isveren_markasi WHERE ust_kt='".$sid."'");
			foreach ($sor_ana_sid->fetchAll() as $key => $value) {
				@unlink($file_upload_path.$value["resim"]);
			}
			$sil_d = $db->prepare("DELETE FROM isveren_markasi WHERE id='".$sid."' OR ust_kt='".$sid."'");		
			$sil_d->execute();
			if($sil_d->rowCount() > 0){
				?>
				      <div class="alert alert-success alert-dismissable">
			...
			@unlink($file_upload_path.$sor_sid["resim"]);
				$sil_d = $db->prepare("DELETE FROM isveren_markasi WHERE id='".$sid."'");		
				$sil_d->execute();
			if($sil_d->rowCount() > 0){
				?>
				      <div class="alert alert-success alert-dismissable">
			...
			@unlink($file_upload_path.$value["resim"]);
				...
				if(!empty($_FILES["icerik_resim"]["name"])){
					$sor_sid = $db->query("SELECT * FROM isveren_markasi WHERE id='".$sid."'")->fetch();
						...
						$isim_s = (substr(md5(rand()),0,10).".".$uzanti);
						@copy($_FILES["icerik_resim"]["tmp_name"],$file_upload_path.$isim_s);
						$res_ekle = $db->prepare("UPDATE isveren_markasi SET baslik=:grup_name,icerik=:icerik,resim=:res  WHERE id=:id");
						...
						$res_ekle = $db->prepare("UPDATE isveren_markasi SET baslik=:grup_name,icerik=:icerik  WHERE id=:id");
						$res_ekle->bindValue(":grup_name",$baslik);
						$res_ekle->bindValue(":icerik",$icerik);
						//$res_ekle->bindValue(":res",$isim_s);
						$res_ekle->bindValue(":id",$sid);
		      ...
		      <div class="alert alert-success alert-dismissable">
		                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
		                    <h4>	<i class="icon fa fa-check"></i> Başarılı!</h4>
		                    Finans kategorisi başarıyla güncellendi listeden detaylı görüntüleme yapabilirsiniz.
		                  </div>
					<?php
				}else{
					?>
		    		 <div class="alert alert-warning alert-dismissable">
		                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
		                    <h4><i class="icon fa fa-warning"></i> Hata!</h4>
		                   Finans kategorisi güncellenemiyor lütfen yetkililere sorun hakkında bilgilendirme yapınız.
		                  </div>
					<?php
				}
