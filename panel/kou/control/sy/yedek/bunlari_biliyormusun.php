...
if($sid==0 && (!isset($islem) || $islem != "sil")){
		...
		$res_ekle = $db->prepare("INSERT INTO videolar(baslik,icerik,sira,aciklama) values(:baslik,:icerik,:sira,:aciklama)");
		$res_sira = $db->query("SELECT MAX(sira) FROM videolar")->fetch();
		$res_ekle->bindValue(":baslik",$baslik);
		$res_ekle->bindValue(":icerik",$icerik);
		$res_ekle->bindValue(":aciklama",$aciklama);
      ...
      <div class="alert alert-success alert-dismissable">
		...
		*/
		$sil_d = $db->prepare("DELETE FROM videolar WHERE id='".$sid."'");
		$sil_d->execute();
...
*/
		...
		$sor_sid = $db->query("SELECT * FROM videolar WHERE id='".$sid."'")->fetch();

				$res_ekle = $db->prepare("UPDATE videolar SET baslik=:baslik,icerik=:icerik,aciklama=:aciklama  WHERE id=:id");
				$res_ekle->bindValue(":baslik",$baslik);
				$res_ekle->bindValue(":icerik",$icerik);
				$res_ekle->bindValue(":aciklama",$aciklama);
